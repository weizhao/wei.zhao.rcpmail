...
public class Message extends ModelObject {
	...
	/* default */void setParent(Folder parent) {
		firePropertyChange("parent", this.parent, this.parent = parent);
	}

	// FG can we get rid of this method because it does not make sense in the
	// domain model?
	// FRU How do we get the folder from the message then?
	...
	public static Message createExample(int i) {
		...
		Message message = new Message(subject, from, new Date().toString(),
				body);
